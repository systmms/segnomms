...
class ReviewSuite:
    ...
    def generate_review_pages(self) -> None:
        ...
        visual_stats = {"pending": len(visual_tests), "approved": 0, "rejected": 0}

        # Generate dashboard
        print("  📄 Generating dashboard...")
        dashboard_html = self.generator.generate_dashboard(
            test_results=test_results, visual_stats=visual_stats, recent_activity=self._get_recent_activity()
        )
    ...
    def serve(self, port: int = 8080) -> None:
        ...
        try:
            subprocess.run(["python", "-m", "http.server", str(port)], cwd=str(self.output_dir), check=True)
...
def main():
    ...
    parser.add_argument("--serve", action="store_true", help="Serve the review suite after generation")
    parser.add_argument("--port", type=int, default=8080, help="Port to serve on (default: 8080)")
    parser.add_argument(
        "--no-generate", action="store_true", help="Skip generation and only serve existing files"
    )
