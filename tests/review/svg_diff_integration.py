...
from tests.helpers.test_case_generator import TestCaseGenerator, Case
...
class SVGDiffIntegration:
    ...
    def __init__(self, output_manager: Optional[OutputManager] = None):
        ...
        """
        self.output_manager = output_manager or TestOutputManager()
    ...
    def create_diff_summary(self) -> Dict[str, Any]:
        ...
        both_changed = sum(1 for c in comparisons 
                          if c["svg"]["has_changes"] and c["png"]["has_changes"])
        svg_only = sum(1 for c in comparisons 
                      if c["svg"]["has_changes"] and not c["png"]["has_changes"])
        png_only = sum(1 for c in comparisons 
                      if not c["svg"]["has_changes"] and c["png"]["has_changes"])
        ...
        for test_case in all_cases:
            if test_case.id == test_id:
                return {
                    ...
                    "tags": test_case.tags,
                    "qr_data": test_case.qr_data[:50] + "..." if len(test_case.qr_data) > 50 else test_case.qr_data
                }
