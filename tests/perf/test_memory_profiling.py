...
class TestMemoryProfiling:
    ...
    @pytest.mark.slow
    def test_matrix_manipulator_memory_usage(self, profiler, qr_matrices):
        ...
        for size_name, matrix in qr_matrices.items():
            ...
            centerpiece = manipulator.get_centerpiece_bounds()
            profiler.take_snapshot(f"{size_name}_get_centerpiece")
        ...
        assert not leak_result["leak_detected"], \
            f"Potential memory leak detected: {leak_result}"
            
        # Report memory usage
        self._report_memory_analysis("MatrixManipulator", analysis)

    @pytest.mark.slow
    def test_clustering_memory_usage(self, profiler, qr_matrices):
        ...
        assert not leak_result["leak_detected"], \
            f"Clustering memory leak: {leak_result}"
            
        self._report_memory_analysis("Clustering", analysis)

    @pytest.mark.slow
    def test_config_creation_memory_usage(self, profiler):
        ...
        assert not leak_result["leak_detected"], \
            f"Config creation memory leak: {leak_result}"
            
        self._report_memory_analysis("ConfigCreation", analysis)

    def test_memory_intensive_operations(self, profiler):
        ...
        assert memory_increase < 20, \
            f"Memory not properly released: {memory_increase:.1f}MB still allocated"
            
        self._report_memory_analysis("IntensiveOperations", analysis)

    def test_repeated_operations_memory_stability(self, profiler):
        ...
        assert analysis["total_growth_mb"] < 10, \
        ...
        assert not leak_result["leak_detected"], \
            f"Memory not being released properly: {leak_result}"
            
        self._report_memory_analysis("RepeatedOperations", analysis)

    def _report_memory_analysis(self, test_name: str, analysis: Dict[str, Any]):
        ...
        if growth_periods:
            ...
            for period in growth_periods:
                print(f"  {period['from_operation']} -> {period['to_operation']}: "
...
class TestMemoryLeakDetection:
    ...
    def test_no_memory_leaks_in_basic_operations(self):
        ...
        for i in range(100):
            ...
            matrix = [[True, False] * 10 for _ in range(21)]
            detector = Mock(spec=ModuleDetector)
            detector.get_module_type.return_value = "data"
        ...
        gc.collect()
        final_snapshot = profiler.take_snapshot("leak_test_final")
        
        leak_result = profiler.detect_memory_leak(baseline, threshold_mb=5)
        
        assert not leak_result["leak_detected"], \
            f"Memory leak in basic operations: {leak_result}"

    def test_clustering_memory_release(self):
        ...
        leak_result = profiler.detect_memory_leak(baseline, threshold_mb=8)
        assert not leak_result["leak_detected"], \
            f"Memory leak in clustering: {leak_result}"