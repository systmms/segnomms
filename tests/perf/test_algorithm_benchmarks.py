...
class TestAlgorithmBenchmarks:
    ...
    def qr_matrices(self):
        ...
        size_small = 21
        matrices['small'] = [[i % 2 == 0 for i in range(size_small)] 
                           for _ in range(size_small)]
        
        # Medium QR (Version 10: 57x57)  
        size_medium = 57
        matrices['medium'] = [[i % 2 == 0 for i in range(size_medium)]
                            for _ in range(size_medium)]
        
        # Large QR (Version 40: 177x177)
        size_large = 177
        matrices['large'] = [[i % 3 == 0 for i in range(size_large)]
                           for _ in range(size_large)]
    ...
    def test_matrix_manipulator_benchmarks(self, benchmarker, qr_matrices, mock_detectors):
        ...
        for size_name, matrix in qr_matrices.items():
            ...
            result = benchmarker.benchmark_function(
                MatrixManipulator,
                matrix, detector,
                name=f"MatrixManipulator_create_{size_name}"
            )
            ...
            result = benchmarker.benchmark_function(
                manipulator.get_module_bounds,
                name=f"get_module_bounds_{size_name}"
            )
            results.append(result)
        ...
        for mode in connectivity_modes:
            ...
            for size_name, matrix in qr_matrices.items():
                ...
                result = benchmarker.benchmark_function(
                    analyzer.process,
                    matrix, detector,
                    name=f"clustering_{mode}_{size_name}"
                )
                results.append(result)
    ...
    def test_path_clipper_benchmarks(self, benchmarker):
        ...
        for scenario_name, coordinates in coordinate_sets:
            ...
            def benchmark_scale_factors():
                return [clipper.get_scale_factor(x, y, distance=25) 
                       for x, y in coordinates]
                       
            result = benchmarker.benchmark_function(
                benchmark_scale_factors,
                name=f"path_clipper_scale_{scenario_name}"
            )
            results.append(result)
        ...
        for path_name, path_data in complex_paths.items():
            result = benchmarker.benchmark_function(
                clipper.clip_path,
                path_data,
                name=f"path_clipping_{path_name}"
            )
            results.append(result)
    ...
    def test_intent_processor_benchmarks(self, benchmarker):
        ...
        intent_scenarios = [
            ...
            ("medium", {
                "shape": "rounded", "scale": "medium", "colors": "brand",
                "interactive": True, "centerpiece": True
            }),
            ("complex", {
                "shape": "connected", "scale": "large", "colors": "gradient",
                "interactive": True, "centerpiece": True, "frame": True,
                "advanced": True, "accessibility": True
            }),
            ("conflicting", {
                "shape": "circle", "geometry": {"shape": "square"},
                "scale": "large", "size": {"scale": 1},
                "colors": "red", "style": {"colors": "blue"}
            })
        ]
        
        for scenario_name, intents in intent_scenarios:
            result = benchmarker.benchmark_function(
                processor.process_intents,
                intents,
                name=f"intent_processing_{scenario_name}"
            )
            results.append(result)
    ...
    def test_configuration_creation_benchmarks(self, benchmarker):
        ...
        config_scenarios = [
            ...
            ("comprehensive", {
                "scale": 15, "border": 6, "dark": "#1a1a2e", "light": "#ffffff",
                "shape": "rounded", "corner_radius": 0.3, "merge": "soft",
                "connectivity": "8-way", "interactive": True, "tooltips": True,
                "centerpiece_enabled": True, "centerpiece_size": 0.15,
                "frame_shape": "circle", "frame_clip_mode": "fade"
            }),
            ("kwargs_heavy", {
                "scale": 20, "shape": "connected", "merge": "aggressive",
                "enable_phase1": True, "enable_phase2": True, "enable_phase3": True,
                "accessibility_enabled": True, "patterns_enabled": True,
                "frame_enabled": True, "interactive": True
            })
        ]
        ...
        for scenario_name, kwargs in config_scenarios:
            ...
            result = benchmarker.benchmark_function(
                ...
                **kwargs,
                name=f"config_from_kwargs_{scenario_name}"
            )
            results.append(result)
            ...
            result = benchmarker.benchmark_function(
                RenderingConfig.model_validate,
                config_data,
                name=f"config_model_validate_{scenario_name}"
            )
            results.append(result)
        ...
        for result in results:
            ...
            if "small" in result.name:
                ...
                assert result.mean_time < 0.1, \
                    f"{result.name} too slow for small QR: {result.mean_time:.3f}s"
                ...
                assert result.mean_time < 2.0, \
                    f"{result.name} too slow for large QR: {result.mean_time:.3f}s"
            else:
                # General operations should be fast
                assert result.mean_time < 1.0, \
                    f"{result.name} too slow: {result.mean_time:.3f}s"
...
class TestScalabilityBenchmarks:
    ...
    def test_matrix_manipulator_scaling(self):
        ...
        for version, size in zip(qr_versions, qr_sizes):
            ...
            result = benchmarker.benchmark_function(
                self._matrix_operations_combined,
                matrix, detector,
                name=f"matrix_ops_v{version}_size{size}"
            )
    ...
    def test_clustering_scaling(self):
        ...
        for version, size in zip(qr_versions, qr_sizes):
            ...
            result = benchmarker.benchmark_function(
                analyzer.process,
                matrix, detector,
                name=f"clustering_v{version}_size{size}"
            )
            results.append((version, size, result))
    ...
    def _analyze_scaling_behavior(self, algorithm_name: str, results: List[Tuple]):
        ...
        for version, size, result in results:
            print(f"Version {version:2d} (Size {size:3d}x{size:3d}): "
