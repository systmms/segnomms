...
def find_similar_configs(baseline_dir: Path, threshold: float = 0.9):
    ...
    for i, config1 in enumerate(configs):
        for j, config2 in enumerate(configs[i+1:], i+1):
            similarity = calculate_config_similarity(
                config1["config"], 
                config2["config"]
            )
...
def find_active_baselines(test_files: List[Path]) -> Dict[str, List[str]]:
    ...
    return {
        'svg': list(active_baselines['svg']),
        'config': list(active_baselines['config'])
    }
...
def identify_orphaned_baselines(baseline_dir: Path) -> Dict[str, List[Path]]:
    ...
    orphaned = {
        'svg': [],
        'config': [],
        'pairs': []  # SVG+Config pairs that are both orphaned
    }
    
    # Check SVG files
    for svg_file in existing_svg:
        basename = svg_file.stem.replace('.baseline', '')
        if not any(basename in active_name or active_name in basename 
                  for active_name in active['svg']):
    ...
    for config_file in existing_config:
        ...
        if not any(basename in active_name or active_name in basename 
                  for active_name in active['config']):
...
def main():
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", 
        help="Analyze baseline configurations"
    )
    ...
    regen_parser = subparsers.add_parser(
        "regenerate",
        help="Regenerate baselines from configurations"
    )
    regen_parser.add_argument(
        "config_file",
        type=Path,
        help="Configuration file to regenerate from"
    )
    regen_parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    similar_parser = subparsers.add_parser(
        "find-similar",
        help="Find similar configurations"
    )
    ...
    clean_parser = subparsers.add_parser(
        "clean-orphaned",
        help="Remove orphaned baseline files no longer used by tests"
    )
