...
class PerformanceMonitor:
    ...
    def get_metrics_summary(self, name: str = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_metrics": [
                m.to_dict() for m in filtered_metrics[-5:]  # Last 5 measurements
            ]
        }
    ...
    def generate_performance_report(self, output_file: Optional[Path] = None) -> str:
        ...
        for name in operation_names:
            ...
            report_lines.append(f"  Execution Time:")
            ...
            report_lines.append(f"    Range: {time_stats['min']*1000:.2f} - {time_stats['max']*1000:.2f} ms")
            ...
            if time_stats['mean'] > 1.0:
                report_lines.append(f"    ⚠️  WARNING: High execution time")
            if memory_stats['mean'] > 50:
                report_lines.append(f"    ⚠️  WARNING: High memory usage")
            if time_stats['std_dev'] / time_stats['mean'] > 0.5:
                report_lines.append(f"    ⚠️  WARNING: High variability in execution time")
