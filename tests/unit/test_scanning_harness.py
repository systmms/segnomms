...
from tests.helpers.scanning_harness import (
    QRScanabilityHarness, ScanTestCase, ScanTestResult, ScanResult,
    PIL_AVAILABLE, OPENCV_AVAILABLE, PYZBAR_AVAILABLE
)
...
class TestScanTestCase:
    ...
    def test_scan_test_case_creation_basic(self):
        ...
        test_case = ScanTestCase(
            name="basic_test",
            description="Basic scanning test",
            dpi=150
        )
...
class TestScanTestResult:
    ...
    def test_scan_test_result_error(self):
        ...
        result = ScanTestResult(
            ...
            error_message="Scanner library not available"
        )
...
class TestQRScanabilityHarness:
    ...
    def test_generate_stress_test_cases(self):
        ...
        for test_case in test_cases:
            ...
            if (test_case.scale_factor < 1.0 or 
                test_case.rotation_degrees != 0 or 
                test_case.blur_radius > 0 or 
                test_case.noise_level > 0 or
                test_case.brightness_adjustment != 0.0 or
                test_case.contrast_adjustment != 1.0):
                challenging_cases += 1
    ...
    def test_analyze_results_all_success(self):
        ...
        test_cases = [
            ScanTestCase(f"test_{i}", f"Test {i}", 150) 
            for i in range(3)
        ]
        results = [
            ScanTestResult(
                test_case=tc,
                result=ScanResult.SUCCESS,
                decoded_data="Test Data",
                scan_time_ms=50.0 + i * 10
            )
...
class TestScanningHarnessIntegration:
    ...
    def test_harness_with_real_svg_no_scanners(self, simple_svg_content):
        ...
        with patch.multiple(
            ...
            test_case = ScanTestCase("test", "Test", 150)
            result = harness.validate_qr_with_test_case(
                simple_svg_content, "Test Data", test_case
            )
    ...
    def test_custom_test_case_generation(self):
        ...
        # Low DPI test
        custom_cases.append(ScanTestCase(
            "low_dpi", "Low DPI scanning", 72, scale_factor=0.5
        ))
        
        # High DPI test  
        custom_cases.append(ScanTestCase(
            "high_dpi", "High DPI scanning", 600, scale_factor=1.5
        ))
        
        # Rotated test
        custom_cases.append(ScanTestCase(
            "rotated", "Rotated QR code", 150, rotation_degrees=45
        ))
    ...
    def test_invalid_svg_content(self):
        ...
        with patch.object(harness, 'available_scanners', ['PIL']):
            with patch.object(harness, 'svg_to_image') as mock_convert:
                ...
                result = harness.validate_qr_with_test_case(
                    invalid_svg, "Test Data", test_case
                )
...
class TestScanningHarnessConfiguration:
    ...
    def test_generate_test_cases_with_parameters(self):
        ...
        if hasattr(harness, 'generate_test_cases_with_dpi_range'):
            test_cases = harness.generate_test_cases_with_dpi_range(
                min_dpi=100, max_dpi=400, step=100
            )
