...
class TestStyleIntentErrorHandling:
    ...
    def test_unsupported_shape_creates_warning(self):
        ...
        intents = IntentsConfig(style=StyleIntents(module_shape="invalid-shape"))
    ...
    def test_unsupported_merge_strategy(self):
        ...
        payload = PayloadConfig(text="Test")
        intents = IntentsConfig(style=StyleIntents(merge="ultra-aggressive"))  # Not a real strategy
...
class TestIntentTransformationTracking:
    ...
    def test_transformation_report_includes_errors(self):
        ...
        shape_transforms = [t for t in report.transformation_steps if t.intent_path == "style.module_shape"]
...
class TestErrorRecovery:
    ...
    def test_error_details_in_translation_report(self):
        ...
        payload = PayloadConfig(text="Test")
        intents = IntentsConfig(style=StyleIntents(patterns={"finder": "invalid-pattern"}))
        ...
        pattern_steps = [
            s for s in result.translation_report.transformation_steps if "patterns" in s.intent_path
        ]
