...
from segnomms.config import ConnectivityMode, MergeStrategy, ModuleShape
...
qr_size_strategy = st.integers(min_value=21, max_value=177).filter(lambda n: (n - 21) % 4 == 0)
...
class TestPropertyBased:
    ...
    def test_basic_generation_never_crashes(self, data: str, shape: str, scale: int, safe_mode: bool):
    ...
    def test_geometry_configs_produce_valid_svg(self, data: str, error_level: str, geometry_config: dict):
    ...
    def test_safe_mode_creates_valid_qr(self, shape: str, safe_mode: bool, scale: int):
        ...
        if safe_mode:
            ...
            assert (
                svg_content.count('class="qr-finder"') >= 3
                or svg_content.count("finder") >= 3
                or "M0" in svg_content
            )  # Path-based finders
    ...
    def test_island_removal_consistency(self, connectivity: str, merge: str, min_island: int):
