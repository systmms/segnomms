...
from segnomms.core.detector import ModuleDetector, FINDER_PATTERN_POSITIONS, FINDER_SIZE, ALIGNMENT_PATTERN_SIZE
from segnomms.core.models import ModuleDetectorConfig, NeighborAnalysis
...
class TestModuleDetectorIntegration:
    ...
    def test_complete_qr_analysis(self):
        ...
        for row in range(7):
            for col in range(7):
                # Top-left finder
                matrix[row][col] = (row in [0, 6] or col in [0, 6] or 
                                  (1 <= row <= 5 and 1 <= col <= 5 and (row in [2, 4] or col in [2, 4])))
