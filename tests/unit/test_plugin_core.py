...
class TestPatternSpecificFunctions:
    ...
    def test_get_pattern_specific_style_with_custom_config(self):
        ...
        config = RenderingConfig(dark=TEST_COLORS["red"], light=TEST_COLORS["white"])
    ...
    def test_get_pattern_specific_render_kwargs_finder(self, basic_config):
        ...
        kwargs = _get_pattern_specific_render_kwargs(basic_config, "finder", {"scale": 10})
    ...
    def test_get_pattern_specific_render_kwargs_custom(self):
        ...
        config = RenderingConfig(finder=FinderConfig(shape=FinderShape.CIRCLE, inner_scale=0.5, stroke=2.0))

        kwargs = _get_pattern_specific_render_kwargs(config, "finder", {"scale": 10})
...
class TestRenderCluster:
    ...
    def test_render_basic_cluster(self, basic_config):
        ...
        _render_cluster(group, cluster, basic_config, detector)
    ...
    def test_render_cluster_with_merge(self):
        ...
        config = RenderingConfig(
            geometry=GeometryConfig(merge=MergeStrategy.SOFT, connectivity=ConnectivityMode.EIGHT_WAY)
        )
        ...
        # _render_cluster modifies the group in-place
        _render_cluster(group, cluster, config, detector)
...
class TestEnhancedRenderKwargs:
    ...
    def test_basic_enhanced_kwargs(self, basic_config):
        ...
        kwargs = _get_enhanced_render_kwargs(basic_config, analysis, "data")
    ...
    def test_enhanced_kwargs_with_geometry(self):
        ...
        config = RenderingConfig(geometry=GeometryConfig(shape=ModuleShape.DIAMOND, corner_radius=0.0))

        # _get_enhanced_render_kwargs takes config, analysis, module_type
        analysis = {"neighbors": [], "neighbor_count": 0}
        kwargs = _get_enhanced_render_kwargs(config, analysis, "data")
...
class TestDetectAndRemoveIslands:
    ...
    def test_detect_islands_enabled(self):
        ...
        result = _detect_and_remove_islands(
            matrix, detector, config.geometry.min_island_modules, config.geometry.connectivity
        )
    ...
    def test_detect_islands_empty_matrix(self):
        ...
        config = RenderingConfig(geometry=GeometryConfig(min_island_modules=1))
...
class TestGenerateConfigHash:
    ...
    def test_generate_hash_different_configs(self):
        ...
        config1 = RenderingConfig(geometry=GeometryConfig(shape=ModuleShape.SQUARE))
        config2 = RenderingConfig(geometry=GeometryConfig(shape=ModuleShape.CIRCLE))
