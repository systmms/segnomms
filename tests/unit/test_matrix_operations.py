...
from segnomms.config import CenterpieceConfig, PlacementMode, ReserveMode
...
class TestMatrixManipulator:
    ...
    def test_is_in_centerpiece_circle(self, manipulator):
        ...
        config = CenterpieceConfig(
            ...
            size=0.2,
            shape="circle",
            margin=0  # Remove margin to get exact size
        )
    ...
    def test_validate_reserve_impact(self, manipulator):
        ...
        small_config = CenterpieceConfig(
            enabled=True,
            size=0.1,  # 10% - should be safe
            shape="circle"
        )
        ...
        large_config = CenterpieceConfig(
            enabled=True,
            size=0.3,  # 30% - likely unsafe for M error correction
            shape="circle"
        )
