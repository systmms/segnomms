...
from segnomms.config.models.geometry import GeometryConfig, FinderConfig
...
class TestGeometryConfig:
    ...
    def test_default_geometry_config(self):
        ...
        assert config.shape == "square"  # use_enum_values=True converts to string
    ...
    def test_shape_string_conversion(self):
        """Test shape string value conversion."""
        # Test string values are stored as strings due to use_enum_values=True
        ...
        assert config.shape == "connected-extra-rounded"
    ...
    def test_connectivity_mode_validation(self):
        ...
        assert config.connectivity == "8-way"
    ...
    def test_merge_strategy_validation(self):
        ...
        assert config.merge == "none"
        
        config = GeometryConfig(merge="soft")
        assert config.merge == "soft"
        
        config = GeometryConfig(merge="aggressive")
        assert config.merge == "aggressive"
...
class TestFinderConfig:
    ...
    def test_default_finder_config(self):
        ...
        assert config.shape == "square"  # use_enum_values=True
...
class TestGeometryFinderIntegration:
    ...
    def test_geometry_finder_shape_independence(self):
        ...
        assert geometry_config.shape == "circle"
        assert finder_config.shape == "square"
        
        # And vice versa
        geometry_config = GeometryConfig(shape="square")
        finder_config = FinderConfig(shape="circle")
        
        assert geometry_config.shape == "square"
        assert finder_config.shape == "circle"
    ...
    def test_coordinated_geometric_styling(self):
        ...
        assert geometry_config.shape == "rounded"
        assert geometry_config.corner_radius == 0.3
        assert finder_config.shape == "rounded"
    ...
    def test_complex_geometry_configurations(self):
        ...
        assert geometry_config.shape == "connected-classy-rounded"
        assert geometry_config.corner_radius == 0.4
        assert geometry_config.connectivity == "8-way"
        assert geometry_config.merge == "aggressive"
