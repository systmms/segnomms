...
from segnomms.color.palette import (
    ColorInfo, ContrastAnalysis, PaletteConfig, PaletteValidationResult,
    validate_palette, generate_palette_suggestions, get_safe_color_alternatives,
    PaletteType, ColorSpace, ContrastStandard
)
from segnomms.config import RenderingConfig, PatternStyleConfig
...
class TestPaletteValidation:
    ...
    def test_color_distribution_analysis(self):
        ...
        config = PaletteConfig(
            dark="black",
            light="#333333",  # Also dark
            accent_colors=["#111111"]  # Also dark
        )
...
class TestRenderingConfigIntegration:
    ...
    def test_disabled_palette_validation(self):
        ...
        config = RenderingConfig(
            dark="yellow",  # Low contrast
            light="white",
            enable_palette_validation=False
        )
    ...
    def test_custom_contrast_requirements(self):
        ...
        config = RenderingConfig(
            dark="gray",
            light="white",
            min_contrast_ratio=3.0  # Lower requirement
        )
