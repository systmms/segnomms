...
from tests.helpers.test_case_generator import TestCaseGenerator, Case, Category
...
class TestVisualRegressionPNG:
    ...
    def _generate_and_test_case(self, test_case: Case):
        ...
        outputs = self.output_manager.generate_test_output(
            test_case.id, qr, test_case.config, "output"
        )
...
class TestVisualRegressionSpecific:
    ...
    def test_centerpiece_variations(self):
        ...
        centerpiece_configs = [
            ...
            ("circle", {"centerpiece_enabled": True, "centerpiece_size": 0.15, "centerpiece_shape": "circle"}),
            ("square", {"centerpiece_enabled": True, "centerpiece_size": 0.15, "centerpiece_shape": "square"}),
        ]
...
class VisualRegressionUtilities:
    ...
    def generate_all_baselines(self):
        ...
        for test_case in all_cases:
            ...
            # Generate baseline
            self.output_manager.generate_test_output(
                test_case.id, qr, test_case.config, "baseline"
            )
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store_true", 
        default=False,
        help="Run only quick visual regression tests"
    )
