...
from segnomms.plugin import (
    ...
    _export_configuration, register_with_segno, MAX_QR_SIZE
)
from segnomms.config import (
    RenderingConfig, GeometryConfig, FinderConfig, StyleConfig,
    FrameConfig, CenterpieceConfig, Phase1Config, Phase2Config, Phase3Config,
...
def full_config():
    ...
    return RenderingConfig(
        ...
        centerpiece=CenterpieceConfig(
            enabled=True,
            size=0.10,  # Reduced to avoid validation error
            shape="circle"
        )
    )
...
class TestGenerateInteractiveSVG:
    ...
    def test_generate_with_phase1_disabled(self, qr_code):
        ...
        config = RenderingConfig(
            phase1=Phase1Config(enabled=False)
        )
...
class TestExportConfiguration:
    ...
    def test_export_with_write_error(self, basic_config):
        ...
        result = _export_configuration(
            basic_config, 
            Path("/non/existent/path/test.svg")
        )
