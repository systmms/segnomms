...
class TestConnectedShapes:
    ...
    def test_shape_comparison_generation(self, tmp_path):
        ...
        for shape in shapes:
            ...
            qr_outputs[shape] = {"file": output_file, "content": svg_content, "size": len(svg_content)}
    ...
    def test_shape_parameter_validation(self, tmp_path):
        ...
        config = create_test_config(shape=ModuleShape.CONNECTED.value, scale=8)
        write(qr, str(output_file), **config)
        assert output_file.exists()
        ...
        try:
            write(qr, str(output_file2), shape="invalid-connected-shape", scale=8)
