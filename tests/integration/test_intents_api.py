...
from segnomms.intents.processor import render_with_intents, process_intents
from segnomms.intents.models import (
    PayloadConfig, IntentsConfig, StyleIntents, FrameIntents, 
    ReserveIntents, AccessibilityIntents, ValidationIntents,
    AdvancedIntents, InteractivityIntents, RenderingResult, PerformanceMetrics
)
...
class TestPublicAPIFunctions:
    ...
    def test_render_with_intents_with_intents(self):
        ...
        intents = IntentsConfig(
            style=StyleIntents(module_shape="squircle"),
            validation=ValidationIntents(quiet_zone=4)
        )
    ...
    def test_process_intents_dict_invalid_payload(self):
        ...
        intents_dict = {
            "payload": {"invalid_field": "value"}  # Missing required text/url
        }
    ...
    def test_process_intents_dict_invalid_intents(self):
        ...
        intents_dict = {
            "payload": {"text": "Hello World"},
            "intents": {
                "invalid_intent_type": {"some": "value"}  # Should be ignored
            }
        }
...
class TestIntentWorkflowIntegration:
    ...
    def test_payload_with_intents_processing_workflow(self):
        ...
        intents = IntentsConfig(
            ...
            accessibility=AccessibilityIntents(
                title="Test QR Code",
                desc="A test QR code for unit testing"
            )
        )
    ...
    def test_complex_intents_workflow(self):
        ...
        intents = IntentsConfig(
            ...
            accessibility=AccessibilityIntents(
                ...
                desc="A complex QR code with multiple intents"
            ),
            ...
            advanced=AdvancedIntents(
                mask_pattern=2,
                structured_append={"enabled": False}
            ),
    ...
    def test_url_payload_workflow(self):
        ...
        intents = IntentsConfig(
            style=StyleIntents(module_shape="squircle"),
            validation=ValidationIntents(quiet_zone=2)
        )
...
class TestPerformanceMetricsIntegration:
    ...
    def test_metrics_timing_accuracy_integration(self):
        ...
        component_sum = (
            ...
            result.metrics.svg_generation_time_ms
        )
    ...
    def test_metrics_with_complex_workflow(self):
        ...
        intents = IntentsConfig(
            ...
            accessibility=AccessibilityIntents(
                title="Metrics Test QR",
                desc="Testing performance metrics"
            )
        )
    ...
    def test_metrics_comparison_across_configurations(self):
        ...
        for quiet_zone in [1, 3, 7]:
            intents = IntentsConfig(
                validation=ValidationIntents(quiet_zone=quiet_zone)
            )
            result = render_with_intents(payload, intents)
...
class TestIntentValidationIntegration:
    ...
    def test_degradation_application_integration(self):
        ...
        intents = IntentsConfig(
            ...
            advanced=AdvancedIntents(
                mask_pattern=4,
                structured_append={"enabled": True}
            )
        )
