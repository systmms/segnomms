...
@pytest.mark.decoder
@pytest.mark.skipif(
    not DECODERS_AVAILABLE,
    reason="No QR decoders available (install zxingcpp, pyzbar, or opencv-python)"
)
class TestDecoderSmoke:
    ...
    def test_basic_shapes_decodable(self, shape: str, safe_mode: bool, tmp_path):
        ...
        if (shape, safe_mode) in known_issues:
            pytest.skip(f"Known decoder compatibility issue: {shape} with safe_mode={safe_mode}. "
    ...
    def test_error_correction_levels(self, error_level: str, tmp_path):
        ...
        if error_level == "L":
            pytest.skip("Known decoder compatibility issue: ECC Level L with connected shapes. "
    ...
    def test_different_scales(self, scale: int, tmp_path):
        ...
        pytest.skip("Known decoder compatibility issue: Rounded modules at various scales have "
    ...
    @pytest.mark.parametrize("geometry_config", [
