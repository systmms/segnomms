...
class TestConfigurationExport:
    ...
    def test_basic_config_export(self, tmp_path):
        ...
        result = write(qr, str(output_file), 
                      scale=12, shape="circle",
                      export_config=True)
    ...
    def test_hash_based_naming(self, tmp_path):
        ...
        result = write(qr, str(output_file),
                      scale=15, shape="squircle", corner_radius=0.3,
...
class TestConfigurationReproducibility:
    ...
    def test_load_and_recreate(self, tmp_path):
        ...
        # Generate original
        result = write(qr, str(tmp_path / "original.svg"),
                      scale=15,
                      shape="squircle", 
                      corner_radius=0.35,
                      dark="#e74c3c",
    ...
    def test_batch_consistency(self, tmp_path):
        ...
        for i, cfg in enumerate(configs):
            ...
            result = write(qr, str(tmp_path / f"batch_{i}.svg"),
                          use_hash_naming=True,
                          export_config=True,
                          **cfg)
        ...
        result = write(qr, str(tmp_path / "batch_0_copy.svg"),
                      use_hash_naming=True,
                      **configs[0])
