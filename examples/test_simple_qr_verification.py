...
def test_shape_variations():
    ...
    for shape, expected_primary in shapes_to_test:
        try:
            ...
            qr = segno.make(
                f"Testing {shape} shape with comprehensive content for adequate QR code size and verification"
            )

            # Generate SVG
            svg_path = f"/tmp/shape_test_{shape}.svg"
            segnomms.write(qr, svg_path, shape=shape, scale=8, border=2, dark="#000000", light="#ffffff")
            ...
            success = (
                analysis["total_elements"] > 50  # Reasonable number of elements
                and primary_element == expected_primary  # Correct element type
                and "#000000" in analysis["colors"]
            )  # Correct color
            ...
            results.append({"shape": shape, "error": str(e), "success": False})
...
def test_corner_radius():
    ...
    for radius in radius_values:
        try:
            ...
            segnomms.write(qr, svg_path, shape="rounded", corner_radius=radius, scale=8, border=2)
            ...
            success = analysis["total_elements"] > 50 and has_curves == expected_curves
            ...
            results.append({"radius": radius, "error": str(e), "success": False})
...
def test_colors():
    ...
    for dark, light, name in color_tests:
        try:
            ...
            segnomms.write(qr, svg_path, shape="square", scale=6, border=2, dark=dark, light=light)
            ...
            success = (
                analysis["total_elements"] > 50 and has_dark_color
            )  # At least dark color should be present
...
def test_interactive_features():
    ...
    for interactive, tooltips, name in interactive_tests:
        try:
            ...
            segnomms.write(
                qr, svg_path, shape="rounded", scale=6, border=2, interactive=interactive, tooltips=tooltips
            )
            ...
            success = (
                ...
                and analysis["has_interactive"] == interactive
                and analysis["has_tooltips"] == tooltips
            )
...
def test_safe_mode():
    ...
    for safe_mode, description in safe_mode_tests:
        try:
            ...
            results.append({"safe_mode": safe_mode, "error": str(e), "success": False})
...
def generate_final_report(all_results):
    ...
    successful_tests = sum(
        len([r for r in results if r.get("success", False)]) for results in all_results.values()
    )
