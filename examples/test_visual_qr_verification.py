...
def verify_expected_features(analysis, expected_features):
    ...
    for feature, expectation in expected_features.items():
        if feature == 'min_modules':
            ...
            has_curves = (analysis['rounded_elements'] > 0 or 
                         analysis['path_with_curves'] > 0)
            ...
            most_common = max(analysis['element_types'].items(), key=lambda x: x[1])[0] if analysis['element_types'] else 'none'
            passed = most_common == expectation
            results[feature] = {
                'passed': passed,
                'message': f"Expected {expectation}, got {most_common}"
            }
...
def run_comprehensive_visual_tests():
    ...
    for safe_mode, name in safe_mode_tests:
        result = create_test_qr_with_verification(
            ...
            {
                'min_modules': 100,
                'primary_element_type': 'rect' if safe_mode else 'path'
            }
        )
        test_results.append(result)
    ...
    for shape in connected_shapes:
        result = create_test_qr_with_verification(
            ...
            {
                'min_modules': 100,
                'primary_element_type': 'path'  # Connected shapes usually use paths
            }
        )
        test_results.append(result)
...
def generate_summary_report(test_results):
    ...
    for category, results in categories.items():
        ...
        for result in results:
            ...
            print(f"        Modules: {analysis.get('total_modules', 0)}, " 
