...
class MatrixManipulator:
    ...
    def __init__(self, matrix: List[List[bool]], detector: ModuleDetector):
        ...
        for i, row in enumerate(matrix):
            if len(row) != expected_size:
                raise ValueError(f"Matrix must be square. Row {i} has length {len(row)}, expected {expected_size}")
    ...
    def get_centerpiece_bounds(self, config) -> Dict[str, int]:
    ...
    def clear_centerpiece_area(self, config) -> List[List[bool]]:
        ...
        try:
            ...
            if hasattr(config, "mode") and config.mode == ReserveMode.IMPRINT:
                result = self.imprint_processor.apply_imprint_mode(config)
    ...
    def validate_reserve_impact(self, config, error_level: str) -> Dict[str, Any]:
    ...
    def validate_centerpiece_configuration(self, config) -> Tuple[bool, List[str]]:
    ...
    def analyze_pattern_preservation(self, config) -> PatternAnalysis:
    ...
    def get_scanability_assessment(
        self, config, modified_matrix: List[List[bool]]
    ...
    def get_centerpiece_metadata(self, config) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "area_info": {
                ...
                "width": area_width,
                "height": area_height,
                "area_ratio": estimated_modules / (len(self.matrix) * len(self.matrix[0])) if self.matrix else 0,
            },
        }
                ...
                for col in range(self.size):
                    if self.geometry.is_in_centerpiece(row, col, config):
                        if self.matrix[row][col]:  # Only dark modules that would be cleared
                            module_type = self.detector.get_module_type(row, col) if self.detector else "unknown"
    ...
    def get_performance_warnings(self, config) -> List[str]:
    ...
    def get_comprehensive_performance_warnings(self, config) -> List[str]:
    ...
    def _is_edge_module(self, row: int, col: int, config) -> bool:
    ...
    def _should_clear_edge_module(
        self, row: int, col: int, config, modified_matrix: List[List[bool]]
    ) -> bool:
